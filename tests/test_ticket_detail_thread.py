...
async def test_replies_shown_oldest_first():
    ...
    ctx.bot_data["seerr"].get_issue.return_value = _issue(comments=[
        IssueComment(author="Kenny", message="looking into it", created_at=""),
        IssueComment(author="Nathan", message="any update?", created_at=""),
    ])
    ...
    assert "Kenny" in text and "Nathan" in text
