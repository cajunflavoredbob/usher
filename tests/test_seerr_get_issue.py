...
def _payload(comments):
    return {
        ...
        "createdBy": {"displayName": "User2", "plexUsername": "user2"},
        "comments": comments,
    }
...
async def test_first_comment_is_description_rest_is_thread(monkeypatch):
    c = _client(monkeypatch, _payload([
        {"message": "Michael won't play", "user": {"displayName": "User2"},
         "createdAt": "2026-06-16T00:00:00.000Z"},
        {"message": "looking into it", "user": {"displayName": "User1"},
         "createdAt": "2026-06-16T01:00:00.000Z"},
        {"message": "any update?", "user": {"displayName": "User2"},
...
async def test_plex_username_fallback_for_author(monkeypatch):
    c = _client(monkeypatch, _payload([
        {"message": "report", "user": {"displayName": "User2"}},
        {"message": "reply", "user": {"plexUsername": "user1_plex"}},
    ]))
    issue = await c.get_issue(42)
    assert issue.comments[0].author == "user1_plex"
...
async def test_empty_thread_comments_skipped(monkeypatch):
    c = _client(monkeypatch, _payload([
        {"message": "report", "user": {"displayName": "User2"}},
        {"message": "   ", "user": {"displayName": "User1"}},   # whitespace-only
        {"message": "real reply", "user": {"displayName": "User1"}},
    ]))
    issue = await c.get_issue(42)
...
async def test_only_original_report_no_replies(monkeypatch):
    c = _client(monkeypatch, _payload([
        {"message": "just the report", "user": {"displayName": "User2"}},
    ]))
    issue = await c.get_issue(42)
